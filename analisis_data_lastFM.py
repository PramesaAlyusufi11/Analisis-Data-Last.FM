...
def visualisasi(df: pd.DataFrame):
    ...
    ax.plot(genre_order, means_e.values, marker="D", lw=2.5, color="#6A1B9A", ms=9, markerfacecolor="white", markeredgewidth=2.5)
    ...
    for (label, col), warna_ in zip(fitur_multi.items(), warna):
        ...
        ax.plot(genre_order, vals_norm.values, marker="o", lw=2, label=label, color=warna_, ms=7)
    ax.set_xticks(range(len(genre_order)))
    ax.set_xticklabels(genre_order, rotation=20, ha="right")
